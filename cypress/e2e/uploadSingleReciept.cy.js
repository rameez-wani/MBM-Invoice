describe('Upload', () => {
    ...
    it('Upload single receipt!', () => {
        ...
        cy.get('#brows-file-btn').selectFile('Taxi/Screenshot 2024-06-19 112257.png', {action: 'drag-drop'})
        cy.get('#Receipt').click();
        cy.wait(2000);
        cy.get('#upload-invoice-by-operator-submit').click();
        cy.wait(10000);
        cy.get('#invoices_list_refresh').click({force: true});
        cy.get('#operator-invoice-code-INV-2820').click();
        cy.wait(3000);
        ...
        cy.get('#invoiceno').invoke('val').then(text => { const invoice_no = text;
          ...
          cy.get('#invoice-items-table').each(($row, index, $rows) => {
              ...
              cy.writeFile('AzureAI_Single_reciept.csv','Invoice details web url '+','+ 'Invoice Number'+','+'VAT no'+ ','+
                'Invoice Date'+','+ 'Supplier Name'+','+'Supplier Address'+','+'Net Amount'+','+'Tax Amount'+','+
                'Gross Amount'+','+'Line Item Number'+','+'Start Date'+','+'End Date'+
                ...
                ','+tax_amount +','+gross_amount+','+itemNo+','+startDate+','+endDate+','+budgetGroup+','+category+','+subCategory+','+description+
